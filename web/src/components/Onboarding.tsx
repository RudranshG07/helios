  ...
  async function createWallet() {
    ...
    try {
      if (getUserId()) {
        ...
        setWallet(me.walletAddress ?? null);
      } else {
        const { walletAddress } = await api.signup();
    ...
    <div className="wizard">
      ...
      {step === 0 && (
        <div className="card pane">
          <h2>Create your self-custody agent wallet</h2>
          <p className="muted">
            Helios trades from a dedicated Trust Wallet agent wallet. The private key is generated locally and never leaves your machine — you stay in
            full custody and can withdraw anytime.
          </p>
          <button className="primary" disabled={busy} onClick={createWallet}>
      ...
      {step === 2 && (
        <div className="card pane">
          <h2>Review &amp; launch</h2>
          <div className="kv"><span>Agent wallet</span><code>{wallet}</code></div>
          <div className="kv"><span>Fund this wallet</span><span>${capital} of BNB/USDT on BSC</span></div>
          ...
          <p className="muted">Once launched, the agent trades autonomously within these rules. You can pause or stop it anytime from the dashboard.</p>
          <div className="row">
